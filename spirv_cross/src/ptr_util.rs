...
pub unsafe fn read_string_from_ptr(ptr: *const std::os::raw::c_char) -> Result<String, ErrorCode> {
    ...
    #[cfg(target_arch = "wasm32")]
    let string = {
        let bytes = emscripten::get_module().read_bytes_into_vec_while(emscripten::Pointer::from_offset(ptr as u32), |byte, _| 0 != byte, false);
...
pub unsafe fn read_from_ptr<T>(ptr: *const T) -> T {
    ...
    let value = {
        ...
        let bytes = emscripten::get_module().read_bytes_into_vec_while(emscripten::Pointer::from_offset(ptr as u32), |_, bytes_read| bytes_read < num_bytes_to_read, false);
